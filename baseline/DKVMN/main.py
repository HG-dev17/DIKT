...
from config import BASICConfig, DKVMNConfig
...
if __name__ == '__main__':
    ...
    model_name = DKVMNConfig.MODEL_NAME
    dim_s = DKVMNConfig.DIM_S
    size_m = DKVMNConfig.SIZE_M
    dropout = DKVMNConfig.DROPOUT
    epoch = DKVMNConfig.NUM_EPOCH
    batch_size = DKVMNConfig.BATCH_SIZE
    max_seq = DKVMNConfig.SEQ_LEN
    min_seq = DKVMNConfig.MIN_SEQ_LEN
    lr = DKVMNConfig.LEARNING_RATE
    ...
    for fold in range(5):
        ...
        model = DKVMN(skill_max, dim_s, size_m, dropout).to(device)
        optimizer = torch.optim.Adam(model.parameters(), lr=lr)
        early_stop_counter = 0
        for epoch in range(epoch):
            train_loss, train_acc, train_auc, train_rmse = run_epoch(True, train_path, model, optimizer,batch_size, min_seq, max_seq)
            print(
            ...
            test_loss, test_acc, test_auc, test_rmse = run_epoch(False, test_path, model, optimizer,batch_size, min_seq, max_seq)
            print(
