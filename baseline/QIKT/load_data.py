...
class KT_Dataset(data.Dataset):
    ...
    def __init__(self, problem_list, skill_list, ans_list, min_problem_num, max_problem_num):
    ...
    def __getitem__(self, index):
        ...
        use_skill = np.zeros((self.max_problem_num, self.pro_numskill), dtype=int)
        ...
        if BASICConfig.DATASET in ("assist09", "xes3g5m", "eedi", "ednet"):
            adjusted_array = []
            for i, row in enumerate(now_skill):
            ...
            use_skill[:num] = np.array(adjusted_array)
        elif BASICConfig.DATASET in ("assist12", "junyi"):
            use_skill[:num] = now_skill
        elif BASICConfig.DATASET == "assist17":
            use_skill[:num] = np.array(now_skill).reshape(-1, 1)
        ...
        return use_problem, use_skill, use_ans, res_mask
