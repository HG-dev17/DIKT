...
from config import BASICConfig, QIKTConfig
...
if __name__ == '__main__':
    ...
    model_name = QIKTConfig.MODEL_NAME
    emb_size = QIKTConfig.EMB_SIZE
    dropout = QIKTConfig.DROPOUT
    mlp_layer_num = QIKTConfig.MLP_LAYER_NUM
    output_mode = QIKTConfig.OUTPUT_MODE
    lr = QIKTConfig.LEARNING_RATE
    batch_size = QIKTConfig.BATCH_SIZE
    max_seq = QIKTConfig.SEQ_LEN
    min_seq = QIKTConfig.MIN_SEQ_LEN
    epoch = QIKTConfig.NUM_EPOCHS
    ...
    for fold in range(5):
        ...
        model = QIKT(pro_max, skill_max, emb_size, dropout, mlp_layer_num, output_mode).to(device)
