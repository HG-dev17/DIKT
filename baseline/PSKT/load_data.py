    ...
    def readData(self):
        ...
        with open(self.path, 'r') as read:
            for index, line in enumerate(read):
                if BASICConfig.DATASET in ["ednet"]:
                    if index % 7 == 0:
                        pass
                    elif index % 7 == 1:
                        pass
                        ...
                        timestamps = line.strip().split(split_char)
                        timestamps_list.append(timestamps)

                elif BASICConfig.DATASET in ["assist12_1", "assist17"]:
                    if index % 7 == 0:
                        pass
                    elif index % 7 == 1:
                        skills = line.strip().split(split_char)
                ...
                elif BASICConfig.DATASET == "junyi":
                    if index % 6 == 0:
                        pass
                    elif index % 6 == 1:
                        skills = line.strip().split(split_char)
        ...
        return problem_list, skill_list, ans_list, timestamps_list


class KT_Dataset(data.Dataset):

    def __init__(self, problem_list, skill_list, ans_list, timestamps_list, min_problem_num, max_problem_num):

        self.problem_list, self.skill_list, self.ans_list, self.timestamps_list = [], [], [], []
        ...
        for (problem, skill, ans, timestamps) in zip(problem_list, skill_list, ans_list, timestamps_list):
            num = len(problem)
            if num < min_problem_num:
                ...
                now_timestamps = timestamps[num - segment * max_problem_num:]
                ...
                if num > segment * max_problem_num:
                    if num - segment * max_problem_num >= min_problem_num:
                        ...
                        self.timestamps_list.append(timestamps[:num - segment * max_problem_num])
                ...
                for i in range(segment):
                    ...
                    item_timestamps = now_timestamps[i * max_problem_num:(i + 1) * max_problem_num]

                    self.problem_list.append(item_problem)
                    self.skill_list.append(item_skill)
                    self.ans_list.append(item_ans)
                    self.timestamps_list.append(item_timestamps)

            else:
                item_problem = problem
                item_skill = skill
                item_ans = ans
                item_timestamps = timestamps
                self.problem_list.append(item_problem)
                self.skill_list.append(item_skill)
                self.ans_list.append(item_ans)
                self.timestamps_list.append(item_timestamps)
    ...
    def __getitem__(self, index):
        ...
        now_timestamps = self.timestamps_list[index]
        ...
        use_timestamps = np.zeros(self.max_problem_num, dtype=int)
        ...
        use_ans[:num] = now_ans
        use_timestamps[:num] = now_timestamps
        use_mask[1:num] = 1
        ...
        use_timestamps = torch.from_numpy(use_timestamps).to(BASICConfig.DEVICE).long()
        res_mask = torch.from_numpy(use_mask != 0).to(BASICConfig.DEVICE)

        return use_problem, use_skill, use_ans, use_timestamps, res_mask


def getLoader(is_train, path, batch_size, min_problem_num, max_problem_num):
    read_data = getReader(path)
    problem_list, skill_list, ans_list, timestamps_list = read_data.readData()

    dataset = KT_Dataset(problem_list, skill_list, ans_list, timestamps_list, min_problem_num,
                         max_problem_num)
