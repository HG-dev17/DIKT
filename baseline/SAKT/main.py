...
from config import BASICConfig, SAKTConfig
...
if __name__ == '__main__':
    ...
    model_name = SAKTConfig.MODEL_NAME
    emb_size = SAKTConfig.EMB_SIZE
    num_attn_heads = SAKTConfig.NUM_ATTN_HEADS
    dropout = SAKTConfig.DROPOUT
    num_en = SAKTConfig.NUM_EN
    learning_rate = SAKTConfig.LEARNING_RATE
    epoch = SAKTConfig.NUM_EPOCHS
    min_seq = SAKTConfig.MIN_SEQ_LEN
    max_seq = SAKTConfig.SEQ_LEN
    batch_size = SAKTConfig.BATCH_SIZE
    ...
    for fold in range(5):
        ...
        model = SAKT(skill_max, max_seq, emb_size, num_attn_heads, dropout, num_en).to(device)
