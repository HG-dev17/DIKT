...
#     def __init__(self, num_q, num_c, seq_len, emb_size, num_attn_heads, dropout, n_blocks):
#         super().__init__()
...
#
#         self.encoder = get_clones(Encoder_block(emb_size, num_attn_heads, num_q, num_c, seq_len, dropout),
#                                       self.num_en)
#
...
#
#     def forward(self, in_ex, in_cat, in_res):
#
#         if self.num_q > 0:
...
#         start_token = torch.tensor([[2]]).repeat(in_res.shape[0], 1).to(BASICConfig.DEVICE)
...
class SAINT(nn.Module):
    def __init__(self, num_q, num_c, seq_len, emb_size, num_attn_heads, dropout, n_blocks):
        ...
        self.decoder = get_clones(Decoder_block(emb_size, 2, num_attn_heads, seq_len, dropout, num_q, num_c),
                                  self.num_de)
    ...
    def forward(self, in_ex, in_cat, in_res, qtest=False):
        ...
        for i in range(self.num_en):
            ...
            in_cat = in_ex
        ## pass through each decoder blocks in sequence
        start_token = torch.tensor([[0]]).repeat(in_res.shape[0], 1).to(
            BASICConfig.DEVICE)  # give start token defalut response to 2
        in_res = torch.cat((start_token, in_res), dim=-1)
