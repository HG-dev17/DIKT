...
def run_epoch(is_train, path, model, optimizer, batch_size, min_problem_num,
    ...
    for i, data in tqdm(enumerate(data_loader), desc='加载中...', ncols=100):
        use_problem, use_skill, use_ans, res_mask, use_problem_diff, use_skill_diff = data

        q = use_problem[:, :-1]
        c = use_skill[:, :-1]
        sd = use_skill_diff[:, :-1]
        qd = use_problem_diff[:, :-1]
        r = use_ans[:, :-1]

        qshft = use_problem[:, 1:]
        cshft = use_skill[:, 1:]
        sdshft = use_skill_diff[:, 1:]
        qdshft = use_problem_diff[:, 1:]
        ...
        if is_train:
            ...
            # 前向传播
            predict = model(q, c, sd, qd, r, qshft, cshft, sdshft, qdshft)
