...
if __name__ == '__main__':
    ...
    model_name = DIMKTConfig.MODEL_NAME
    dropout = DIMKTConfig.DROPOUT
    emb_size = DIMKTConfig.EMB_SIZE
    batch_size = DIMKTConfig.BATCH_SIZE
    difficult_levels = DIMKTConfig.DIFFICULT_LEVELS
    epoch = DIMKTConfig.NUM_EPOCHS
    max_seq = DIMKTConfig.SEQ_LEN
    min_seq = DIMKTConfig.MIN_SEQ_LEN
    learning_rate = DIMKTConfig.LEARNING_RATE
    weight_decay = DIMKTConfig.WEIGHT_DECAY
    ...
    for fold in range(5):
        ...
        model = DIMKT(pro_max, skill_max, dropout, emb_size, batch_size, difficult_levels).to(device)
