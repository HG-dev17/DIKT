...
class KT_Dataset(data.Dataset):
    ...
    def __init__(self, problem_list, skill_list, ans_list, min_problem_num, max_problem_num, skill_max):

        self.problem_list, self.skill_list, self.ans_list = [], [], []
        ...
        for (problem, skill, ans) in zip(problem_list, skill_list, ans_list):
            num = len(problem)
    ...
    def __getitem__(self, index):
        ...
        now_skill_ans = np.array(now_skill, dtype=int) + np.array(self.ans_list[index], dtype=int) * self.skill_max

        # 由于需要统一格式（统一长度）
        use_problem = np.zeros(self.max_problem_num, dtype=int)
        use_skill = np.zeros(self.max_problem_num, dtype=int)
        use_skill_ans = np.zeros(self.max_problem_num, dtype=int)
        ...
        use_skill_ans[:num] = now_skill_ans
        use_ans[:num] = now_ans
        use_mask[1:num] = 1
        ...
        use_skill_ans = torch.from_numpy(use_skill_ans).to(BASICConfig.DEVICE).long()
        use_ans = torch.from_numpy(use_ans).to(BASICConfig.DEVICE).long()
        res_mask = torch.from_numpy(use_mask != 0).to(BASICConfig.DEVICE)

        return use_problem, use_skill, use_skill_ans, use_ans, res_mask


def getLoader(is_train, path, batch_size, min_problem_num, max_problem_num, skill_max):
    read_data = getReader(path)
    problem_list, skill_list, ans_list = read_data.readData()

    dataset = KT_Dataset(problem_list, skill_list, ans_list, min_problem_num,
