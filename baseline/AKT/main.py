...
from config import BASICConfig, AKTConfig
...
if __name__ == '__main__':
    ...
    model_name = AKTConfig.MODEL_NAME
    n_block = AKTConfig.N_BLOCK
    d_model = AKTConfig.D_MODEL
    dropout = AKTConfig.DROPOUT
    kq_same = AKTConfig.KQ_SAME
    l2 = AKTConfig.L2
    model_type = AKTConfig.MODEL_TYPE
    lr = AKTConfig.LR
    batch_size = AKTConfig.BATCH_SIZE
    max_seq = AKTConfig.MAX_SEQ
    min_seq = AKTConfig.MIN_SEQ
    patience = AKTConfig.EARLY_STOP
    epoch = AKTConfig.EPOCH
    ...
    for fold in range(5):
        ...
        model = AKT(n_question=skill_max, n_pid=pro_max, n_blocks=n_block, d_model=d_model, dropout=dropout,
                    kq_same=kq_same, model_type=model_type, l2=l2).to(device)
        optimizer = torch.optim.Adam(model.parameters(), lr=lr, betas=(0.9, 0.999), eps=1e-8)
        early_stop_counter = 0
        for epoch in range(epoch):
            train_loss, train_acc, train_auc, train_rmse = run_epoch(True, train_path, model, optimizer,
                                                                     batch_size, min_seq, max_seq, skill_max)
            print(
