...
def run_epoch(is_train, path, model, optimizer, batch_size, min_problem_num,
              max_problem_num, criterion, skill_max):
    total_loss = []
    total_correct = 0
    total_num = 0
    labels = []
    ...
    for i, data in tqdm(enumerate(data_loader), desc='加载中...', ncols=100):
        ...
        if is_train:
            ...
            # 前向传播
            predict, cl_loss, reg_loss = model.get_cl_loss(use_skill, use_ans, use_problem)
            ...
            pred_loss = F.binary_cross_entropy_with_logits(next_predict, next_true, reduction="mean")

            kt_loss = pred_loss + cl_loss * 0.1 + reg_loss
            ...
            with torch.no_grad():
                predict, *_ = model.predict(use_skill, use_ans, use_problem)
