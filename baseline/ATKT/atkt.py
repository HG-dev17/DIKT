...
class KT_backbone(nn.Module):
    def __init__(self, skill_dim, answer_dim, hidden_dim, output_dim):
        ...
        self.skill_emb = nn.Embedding(self.output_dim+1, self.skill_dim, padding_idx=0)
    ...
    def _get_next_pred(self, res, skill):
        ...
        one_hot = torch.cat((one_hot, torch.zeros(1, self.output_dim).to(BASICConfig.DEVICE)), dim=0)
        next_skill = skill[:, 1:]
