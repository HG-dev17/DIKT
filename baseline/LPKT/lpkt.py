...
class LPKT(nn.Module):
    def __init__(self, n_exercise, n_question, d_a, d_e, d_k, q_matrix, dropout=0.2):
        super().__init__()
        self.d_k = d_k
        self.d_a = d_a
        self.d_e = d_e
        ...
        self.e_embed = nn.Embedding(n_exercise + 1, d_k)
    ...
    def forward(self, e_data, at_data, a_data, it_data):
        ...
        n_at = glo.get_value('n_at')
        n_it = glo.get_value('n_it')

        if self.at_embed is None:
            self.at_embed = nn.Embedding(n_at + 1, self.d_k)
            torch.nn.init.xavier_uniform_(self.at_embed.weight)
            self.at_embed = self.at_embed.to(BASICConfig.DEVICE)

        if self.it_embed is None:
            self.it_embed = nn.Embedding(n_it + 1, self.d_k)
            torch.nn.init.xavier_uniform_(self.it_embed.weight)
            self.it_embed = self.it_embed.to(BASICConfig.DEVICE)

        at_embed_data = self.at_embed(at_data)
        it_embed_data = self.it_embed(it_data)

        a_data = a_data.unsqueeze(-1).repeat(1, 1, self.d_a)

        all_learning = self.linear_1(torch.cat((e_embed_data, at_embed_data, a_data), 2))

        h_pre = nn.init.xavier_uniform_(torch.zeros(self.n_question + 1, self.d_k)).repeat(batch_size, 1, 1).to(
            BASICConfig.DEVICE)
        h_tilde_pre = None
        learning_pre = torch.zeros(batch_size, self.d_k).to(BASICConfig.DEVICE)

        pred = torch.zeros(batch_size, seq_len).to(BASICConfig.DEVICE)
