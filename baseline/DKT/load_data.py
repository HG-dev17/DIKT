            ...
            for index, line in enumerate(read):
                if BASICConfig.DATASET in ["assist09","ednet"]:
                    ...
                    if index % 6 == 0:
                        ...
                        pass
        return skill_list, ans_list


class KT_Dataset(data.Dataset):

    def __init__(self, skill_list, ans_list, min_problem_num, max_problem_num):

        self.skill_list, self.ans_list = [], []
        ...
        for (skill, ans) in zip(skill_list, ans_list):
            num = len(skill)
    ...
    def __getitem__(self, index):
        ...
        return use_skill, use_ans, res_mask


def getLoader(is_train, path, batch_size, min_problem_num, max_problem_num):
    read_data = getReader(path)
    skill_list, ans_list = read_data.readData()

    dataset = KT_Dataset(skill_list, ans_list, min_problem_num,max_problem_num)
