...
def run_epoch(is_train, path, model, optimizer, batch_size, min_problem_num, max_problem_num, skill_max):
    total_loss = []
    total_correct = 0
    total_num = 0
    labels = []
    ...
    for i, data in tqdm(enumerate(data_loader), desc='加载中...', ncols=100):
        ...
        if is_train:
            ...
            # 前向传播
            predict = model(use_skill, use_ans)
            predict = (predict[:, :-1] * one_hot(use_skill[:, 1:], skill_max)).sum(-1)
            # 计算损失
            next_predict = (torch.masked_select(predict, res_mask[:, 1:])).float()
            next_true = (torch.masked_select(use_ans[:, 1:], res_mask[:, 1:])).float()
            kt_loss = binary_cross_entropy(next_predict, next_true)
