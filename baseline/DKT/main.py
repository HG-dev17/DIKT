...
from config import BASICConfig, DKTConfig
...
if __name__ == '__main__':
    ...
    model_name = DKTConfig.MODEL_NAME
    emb_size = DKTConfig.EMB_SIZE
    dropout = DKTConfig.DROPOUT
    learning_rate = DKTConfig.LEARNING_RATE
    batch_size = DKTConfig.BATCH_SIZE
    min_seq = DKTConfig.MIN_SEQ_LEN
    max_seq = DKTConfig.SEQ_LEN
    epoch = DKTConfig.NUM_EPOCHS
    patience = DKTConfig.EARLY_STOP
    ################################################## model training ##################################################
    avg_acc = 0
    avg_auc = 0
    avg_rmse = 0
    ...
    for fold in range(5):
        ...
        model = DKT(skill_max, emb_size, dropout=0.1).to(device)
