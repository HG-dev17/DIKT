...
from config import BASICConfig, GKTConfig
...
if __name__ == '__main__':
    ...
    model_name = GKTConfig.MODEL_NAME
    dropout = GKTConfig.DROPOUT
    batch_size = GKTConfig.BATCH_SIZE
    max_seq = GKTConfig.MAX_SEQ
    min_seq = GKTConfig.MIN_SEQ
    patience = GKTConfig.EARLY_STOP
    epoch = GKTConfig.EPOCH
    hidden_dim = GKTConfig.HIDDEN_DIM
    emb_size = GKTConfig.EMB_SIZE
    graph_type = GKTConfig.GRAPH_TYPE
    learning_rate = GKTConfig.LEARNING_RATE
    ...
    for fold in range(5):
        ...
        graph = get_gkt_graph(skill_max, dataset, fold)
        ...
        model = GKT(skill_max, hidden_dim, emb_size, graph_type, dropout, graph, bias=True).to(device)
