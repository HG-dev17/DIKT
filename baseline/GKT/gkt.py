...
class GKT(nn.Module):
    ...
    def __init__(self, num_c, hidden_dim, emb_size, graph_type, dropout, graph=None, bias=True):
        ...
        one_hot_feat = torch.eye(self.res_len * self.num_c).to(BASICConfig.DEVICE)
        ...
        self.one_hot_q = torch.eye(self.num_c).to(BASICConfig.DEVICE)
        zero_padding = torch.zeros(1, self.num_c).to(BASICConfig.DEVICE)
        ...
        self.emb_c = nn.Embedding(num_c + 1, emb_size, padding_idx=0)
    ...
    def _aggregate(self, xt, qt, ht, batch_size):
        ...
        """
        qt_mask = torch.ne(qt, 0)  # [batch_size], qt != -1
        x_idx_mat = torch.arange(self.res_len * self.num_c, device=BASICConfig.DEVICE)
        x_embedding = self.interaction_emb(
            x_idx_mat)  # [res_len * num_c, emb_size]#the emb for each concept with answer?
        # print(xt[qt_mask])
        # print(self.one_hot_feat)
        masked_feat = F.embedding(xt[qt_mask],
                                  self.one_hot_feat)  # [mask_num, res_len * num_c] A simple lookup table that looks up embeddings in a fixed dictionary and size.
        ...
        concept_idx_mat = self.num_c * torch.ones((batch_size, self.num_c), device=BASICConfig.DEVICE).long()
        concept_idx_mat[qt_mask, :] = torch.arange(self.num_c, device=BASICConfig.DEVICE)
        concept_embedding = self.emb_c(concept_idx_mat)  # [batch_size, num_c, emb_size]

        index_tuple = (torch.arange(mask_num, device=BASICConfig.DEVICE), qt[qt_mask].long())
    ...
    def _agg_neighbors(self, tmp_ht, qt):
        ...
        neigh_ht = torch.cat((expanded_self_ht, masked_tmp_ht),
                             dim=-1)  # [mask_num, num_c, 2 * (hidden_dim + emb_size)]
    ...
    def _update(self, tmp_ht, ht, qt):
        ...
        m_next, concept_embedding, rec_embedding, z_prob = self._agg_neighbors(tmp_ht,
                                                                               qt)  # [batch_size, num_c, hidden_dim]
        # Erase & Add Gate
        ...
        res = self.gru(m_next[qt_mask].reshape(-1, self.hidden_dim),
                       ht[qt_mask].reshape(-1, self.hidden_dim))  # [mask_num * num_c, hidden_num]
    ...
    def _get_next_pred(self, yt, q_next):
        ...
        """
        next_qt = q_next
        next_qt = torch.where(next_qt != 0, next_qt, self.num_c * torch.ones_like(next_qt, device=yt.device))
    ...
    def forward(self, q, r):
        ...
        ht = Variable(torch.zeros((batch_size, self.num_c, self.hidden_dim), device=BASICConfig.DEVICE))

        pred_list = []
        for i in range(seq_len):
            xt = features[:, i]  # [batch_size]
            qt = questions[:, i]  # [batch_size]
            qt_mask = torch.ne(qt, 0)  # [batch_size], next_qt != -1
