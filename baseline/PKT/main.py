...
from pkt import DKTInSkillQues
...
if __name__ == '__main__':
    ...
    model_name = PKTConfig.MODEL_NAME
    sz_rnn_in = PKTConfig.SZ_RNN_IN
    sz_rnn_out = PKTConfig.SZ_RNN_OUT
    n_rnn_layer = PKTConfig.N_RNN_LAYER
    rnn_dropout = PKTConfig.RNN_DROPOUT
    lr = PKTConfig.LR
    epoch = PKTConfig.N_EPOCH
    batch_size = PKTConfig.BATCH_SIZE
    patience = PKTConfig.EARLY_STOP
    n_skillPad = skill_max + 1
    ...
    for fold in range(5):
        ...
        model = DKTInSkillQues(nSkill=skill_max, nQues=pro_max, szRnnIn=sz_rnn_in, szRnnOut=sz_rnn_out,
                               nRnnLayer=n_rnn_layer, szOut=n_skillPad, dropout=rnn_dropout).to(device)
        optimizer = torch.optim.Adam(model.parameters(), lr=lr)

        early_stop_counter = 0
        for epoch in range(epoch):
            train_loss, train_acc, train_auc, train_rmse = run_epoch(True, train_path, model, optimizer,
                                                                     batch_size, pro_max, skill_max, device)
            print(
