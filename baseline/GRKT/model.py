...
class GRKT(nn.Module):
    ...
    def __init__(self, device, pro_max, skill_max, train_graph_path):
        super().__init__()

        self.train_graphs = np.load(train_graph_path)

        self.skill_max = skill_max
        self.pro_max = pro_max

        self.pos_mode = GRKTConfig.POS_MODE
        self.k_hidden = GRKTConfig.K_HIDDEN
        self.d_hidden = GRKTConfig.D_HIDDEN
        self.k_hop = GRKTConfig.K_HOP
        self.tau = GRKTConfig.TAU
        self.thresh = GRKTConfig.THRESH
        self.alpha = GRKTConfig.ALPHA

        self.init_hidden = nn.Parameter(torch.randn(skill_max, self.k_hidden))
        self.know_master_proj = Positive_Linear(self.k_hidden, 1, 'softmax')
        self.know_embedding = nn.Embedding(skill_max, self.d_hidden, padding_idx=0)
        self.prob_embedding = nn.Embedding(pro_max, self.d_hidden, padding_idx=0)

        self.req_matrix = nn.Linear(self.d_hidden, self.d_hidden, bias=False)
        self.rel_matrix = nn.Linear(self.d_hidden, self.d_hidden, bias=False)

        self.agg_rel_matrix = nn.ModuleList([Positive_Linear(
            self.k_hidden, self.k_hidden, 'softmax') for _ in range(self.k_hop)])
        ...
        self.gain_ffn = nn.Sequential(
            nn.Linear(2 * self.d_hidden + self.k_hidden, self.d_hidden),
            nn.ReLU(),
            nn.Linear(self.d_hidden, self.k_hidden),
        )

        self.gain_matrix_rel = nn.ModuleList(
            [nn.Linear(self.k_hidden, self.k_hidden, bias=False) for _ in range(self.k_hop)])
        ...
        rel_map = graphs['similar']
        self.rel_map = torch.BoolTensor(rel_map).to(device)  # [NK, NK]

        pre_map = graphs['transition']
        self.pre_map = torch.BoolTensor(pre_map).to(device)  # [NK, NK]
        ...
        self.decision_mlp = nn.Sequential(
            nn.Linear(4 * self.d_hidden + self.k_hidden, 2 * self.d_hidden),
            nn.ReLU(),
            nn.Linear(2 * self.d_hidden, 2),
        )

        self.learn_mlp = nn.Sequential(
            nn.Linear(4 * self.d_hidden + self.k_hidden, 2 * self.d_hidden),
            nn.ReLU(),
            nn.Linear(2 * self.d_hidden, self.k_hidden),
        )
        ...
        self.learn_kernel_matrix_rel = nn.ModuleList(
            [nn.Linear(self.d_hidden, self.k_hidden, bias=False)] + \
            [nn.Linear(self.k_hidden, self.k_hidden, bias=False) for _ in range(self.k_hop - 1)])
    ...
    def forward(self, use_oriskill, use_problem, use_ans, use_time):

        B, S, K = use_oriskill.size()
        DH, KH, NK = self.d_hidden, self.k_hidden, self.skill_max

        h = self.init_hidden.repeat(B, 1, 1)  # [B, NK, KH]
        h_initial = h  # [B, NK, KH]

        total_know_embedding = self.know_embedding.weight  # [NK, DH]
        prob_embedding = self.prob_embedding(use_problem)  # [B, S, DH]
        know_embedding = self.know_embedding(use_oriskill)  # [B, S, K, DH]
        ...
        learn_kernel_para = F.softplus(lk_tilde) * self.alpha  # [NK, KH]
        ...
        forget_kernel_para = F.softplus(fk_tilde) * self.alpha  # [NK, KH]
        ...
        for i in range(S):
            ...
            prob = use_problem[:, i]  # [B]
            time = use_time[:, i]
            know = use_oriskill[:, i]  # [B, K]
            corr = use_ans[:, i]  # [B]
            ...
            if i != S - 1:
                ...
                new_know = use_oriskill[:, i + 1]  # [B, K]
                new_time = use_time[:, i + 1]  # [B, K]
                ...
                decision_gumbel_mask = F.gumbel_softmax(decision, tau=self.tau, hard=True, dim=-1)  # [B, 2*K, 2]
                ...
                learn_exp = (-(learn_count[:, :,
                               None].float() + 1) * delta_time * learn_kernel_para_1).exp()  # [B, NK, KH]
                forget_exp = (-(learn_count[:, :,
                                None].float() + 1) * delta_time * forget_kernel_para_1).exp()  # [B, NK, KH]
