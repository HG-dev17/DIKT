...
from model import GRKT
from run import run_epoch
from config import BASICConfig, GRKTConfig
...
if __name__ == '__main__':
    ...
    model_name = GRKTConfig.MODEL_NAME
    batch_size = GRKTConfig.BATCH_SIZE
    min_seq = GRKTConfig.MIN_SEQ
    max_seq = GRKTConfig.MAX_SEQ
    patience = GRKTConfig.EARLY_STOP
    epoch = GRKTConfig.EPOCH
    ################################################## model training ##################################################
    avg_acc = 0
    avg_auc = 0
    avg_rmse = 0
    ...
    for fold in range(5):
        ...
        model = GRKT(device, pro_max, skill_max, train_graph_path).to(device)
        optimizer = torch.optim.Adam(model.parameters(), lr=GRKTConfig.LEARNING_RATE, weight_decay=GRKTConfig.L2,
                                     capturable=True)
        criterion = nn.BCELoss()
        early_stop_counter = 0
        for epoch in range(epoch):
            train_loss, train_acc, train_auc, train_rmse = run_epoch(True, train_path, model, optimizer,
                                                                     batch_size, min_seq, max_seq, criterion)
            print(
