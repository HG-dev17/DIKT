...
class simpleKT(nn.Module):
    def __init__(self, n_question, n_pid, d_model, n_blocks, dropout, d_ff, seq_len, kq_same, final_fc_dim,
                 final_fc_dim2, num_attn_heads, separate_qa, l2, emb_type):
        super().__init__()
        """
        Input:
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, mask, query, key, values, apply_pos=True):
        ...
        src_mask = (torch.from_numpy(nopeek_mask) == 0).to(BASICConfig.DEVICE)
...
def attention(q, k, v, d_k, mask, dropout, zero_pad):
    ...
    # print(zero_pad)
    if zero_pad:
        pad_zero = torch.zeros(bs, head, 1, seqlen).to(BASICConfig.DEVICE)
