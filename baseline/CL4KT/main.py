...
if __name__ == '__main__':
    ...
    model_name = CL4KTConfig.MODEL_NAME
    learning_rate = CL4KTConfig.LEARNING_RATE
    l2 = CL4KTConfig.L2
    patience = CL4KTConfig.EARLY_STOP
    epoch = CL4KTConfig.NUM_EPOCHS
    batch_size = CL4KTConfig.BATCH_SIZE
    eval_batch_size = CL4KTConfig.EVAL_BATCH_SIZE
    max_seq = CL4KTConfig.SEQ_LEN
    mask_prob = CL4KTConfig.MASK_PROB
    crop_prob = CL4KTConfig.CROP_PROB
    permute_prob = CL4KTConfig.PERMUTE_PROB
    replace_prob = CL4KTConfig.REPLACE_PROB
    negative_prob = CL4KTConfig.NEGATIVE_PROB
    ...
    for fold in range(5):
        ...
        for epoch in range(epoch):
            ...
            test_loss, test_acc, test_auc, test_rmse = run_epoch(False, test_path, model, optimizer,
                                                                 eval_batch_size, criterion, max_seq,
                                                                 mask_prob, crop_prob, permute_prob,
                                                                 replace_prob, negative_prob)
            print(
